...
\textbf{Ablation sampling inconsistency.}
The length-constrained ablation reported in
Section~\ref{sec:discussion} was generated using a fresh 90/10
split of the raw, non-deduplicated dataset
(\texttt{random\_state=42}), yielding 397 samples, rather than
the deduplicated 262-sample test set used for the primary RQ1
evaluation. Consequently, the ablation samples are not
guaranteed to be identical to the RQ1 test set at the instance
level. The reported ROUGE-1 and SBERT shifts
(0.491$\rightarrow$0.532 and 0.632$\rightarrow$0.609,
respectively) should therefore be interpreted as indicative of
the length--style relationship on a comparable sample rather
than as a strictly paired before/after comparison.
